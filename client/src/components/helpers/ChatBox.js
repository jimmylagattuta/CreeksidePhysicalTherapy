  ...
  const fetchReviews = () => {
    const url =
      process.env.NODE_ENV === 'production'
        ? 'https://creeksidephysicaltherapy.com/api/v1/pull_google_places_cache'
...
useEffect(() => {
  ...
  const onSubmit = async (values) => {
    ...
    const baseUrl = process.env.NODE_ENV === 'production' ? 'https://creeksidephysicaltherapy.com' : 'http://localhost:3001';
    try {
